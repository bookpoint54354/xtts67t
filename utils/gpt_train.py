...
def train_gpt(custom_model, version, language, num_epochs, batch_size, grad_acumm, train_csv, eval_csv, output_path, max_audio_length=255995):
    ...
    # XTTS transfer learning parameters: You need to provide the paths of XTTS model checkpoint that you want to fine-tune.
    ...
    trainer = Trainer(
        TrainerArgs(
            restore_path=None,  # XTTS checkpoint is restored via xtts_checkpoint key so no need to restore it using Trainer restore_path parameter
            skip_train_epoch=False,
            start_with_eval=START_WITH_EVAL,
            grad_accum_steps=GRAD_ACUMM_STEPS,
            mixed_precision='bf16',  # Added this line for bfloat16 precision
        ),
        config,
        output_path=OUT_PATH,
        model=model,
